...
| **TOOLS.md** | 环境配置 | 用户偏好 |
| **MEMORY.md** | 经验教训、原则、长期入口 | 具体事件流水、一次性任务内容 |
| **memory/YYYY-MM-DD.md** | 每日活动流水 | 需要长期记住的原则 |
| **memory/xxx.md** | 详细技术文档 | 一次性任务文档 |

**关键区分**：
- **长期会用到** → 在 MEMORY.md 写入口，指向 memory/xxx.md
- **一次性任务** → 只在 memory/YYYY-MM-DD.md 记录，不写入口
...
- **任何让用户"干等"的任务都应该派 subagent**——因为 turn-based 机制下，用户无法在执行期间对话

### Subagent 使用规范

**错误模式**：实时监督 AI 过程，随时准备打断纠错
- 假设你能判断每一步对不对
- 消耗你的带宽
- 你可能不如 AI 懂细节
...
**正确模式**：沙盒 + 超时 + 汇报 → 你决策
```
沙盒环境 + 时间限制 + 任务 → AI 造 → 汇报结果 → 你决策
```

**标准流程**：
1. 给沙盒（让它随便造，最坏情况删掉重来）
2. 给时间限制（如 30 分钟）
3. 给任务
4. 去干别的
5. 汇报后你做决策：继续？调整方向？放弃？
